...
function printHelp(): void {
  ...
  bumps the package version (via bumpp), updates CHANGELOG.md, and creates
  ...
  Options:
    ...
    --push             Push the release commit and tag to the remote after creating them.
...
async function main(): Promise<void> {
  ...
  consola.info(`Released ${tagName}.`);

  if (flags.push) {
    runGit(['push', '--follow-tags'], cwd);
    consola.info('Pushed the release commit and tag to the remote.');
  } else {
    consola.info(`Run "git push --follow-tags" to publish ${tagName}.`);
  }
}
